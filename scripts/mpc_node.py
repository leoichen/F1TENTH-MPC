...
class mpc_config:
    ...
    Rdk: list = field(
        default_factory=lambda: np.diag([0.01, 1.])
    )  # input difference cost matrix, penalty for change of inputs - [accel, steering]
    Qk: list = field(
        default_factory=lambda: np.diag([14.5, 14.5, 6., 14.5])
    ...
    MIN_STEER: float = -0.41  # maximum steering angle [rad]
    MAX_STEER: float = 0.41  # maximum steering angle [rad]
...
class MPC(Node):
    ...
    def __init__(self):
        ...
        wps = np.genfromtxt(self.cwd+'upd_lev_wps1.csv', delimiter = ',')
        uniques, ind = np.unique(wps, axis=0, return_index=True)
        # print('uniques', uniques)
        idxs = np.sort(ind)
        self.waypoints = wps[idxs] # x, y, yaw, speed
        ...
        self.vis_pub_ = self.create_publisher(MarkerArray, "/vis_array", 10)
    ...
    def pose_callback(self, pose_msg):
        ...
        vehicle_state.v = (pose_msg.twist.twist.linear.x**2+pose_msg.twist.twist.linear.y**2+pose_msg.twist.twist.linear.z**2)**0.5
        vehicle_state.yaw = self.get_yaw(pose_msg)
        
        print('states', vehicle_state.x, vehicle_state.y, vehicle_state.v,\
            vehicle_state.yaw)
        ...
        ref_v = self.waypoints[:,3] * 0.6
        
        ref_path = self.calc_ref_trajectory(vehicle_state, ref_x, ref_y, ref_yaw, ref_v)
        x0 = [vehicle_state.x, vehicle_state.y, vehicle_state.v, vehicle_state.yaw]
        x0 = [pose_msg.pose.pose.position.x, pose_msg.pose.pose.position.y, \
            (pose_msg.twist.twist.linear.x**2+pose_msg.twist.twist.linear.y**2+pose_msg.twist.twist.linear.z**2)**0.5, \
               self.get_yaw(pose_msg) ]
        ...
        print('steer speed', steer_output, speed_output)
        drive = AckermannDrive(speed = speed_output, \
    ...
    def mpc_prob_init(self):
        ...
        constraints.append(cvxpy.vec(cvxpy.abs(cvxpy.diff(self.uk[1,:]))) <= self.config.MAX_DSTEER * self.config.DTK)
        ...
        constraints.append(cvxpy.vec(self.xk[2,:]) >= self.config.MIN_SPEED)
        constraints.append(cvxpy.vec(self.xk[2,:]) <= self.config.MAX_SPEED)
        constraints.append(cvxpy.vec(cvxpy.abs(self.uk[0,:])) <= self.config.MAX_ACCEL)
        constraints.append(cvxpy.vec(cvxpy.abs(self.uk[1,:])) <= self.config.MAX_STEER)
        # -------------------------------------------------------------
    ...
    def calculate_speed(self, pose_msg):
        if self.prev_msg is None:
            # dx = pose_msg.pose.pose.position.x
            # dy = pose_msg.pose.pose.position.y
            # dz = pose_msg.pose.pose.position.z
            # prev_time = 0

            speed = self.config.MIN_SPEED
        else:
            dx = pose_msg.pose.pose.position.x - self.prev_msg.pose.pose.position.x
            dy = pose_msg.pose.pose.position.y - self.prev_msg.pose.pose.position.y
            dz = pose_msg.pose.pose.position.z - self.prev_msg.pose.pose.position.z
            prev_time = self.prev_msg.header.stamp.sec + \
                (self.prev_msg.header.stamp.nanosec * 10**-9)

            current_time = self.get_clock().now().to_msg().sec + \
                ( self.get_clock().now().to_msg().nanosec * 10**-9)
    ...
    def draw_waypoints(self):
        ...
        # draw waypoints
        for [x,y, yaw, v] in self.waypoints:
